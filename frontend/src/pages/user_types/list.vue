                ...
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-12 md:col-3 " >
                            <template v-if="auth.canView('/user_types/add')">
                                <router-link :to="`/user_types/add`">
                                    <Button label="Add New User Types" icon="pi pi-plus" type="button" class="p-button bg-primary "  />
                                </router-link>
                            </template>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/user_types'}" :model="pageBreadCrumb" />
                            </template>
                                        ...
                                        <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                            ...
                                            <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                                ...
                                                <Column  field="id" header="Id" >
                                                    <template #body="{data}">
                                                        <router-link :to="`/user_types/view/${data.id}`">
                                                            {{ data.id }}
                                                        </router-link>
                                                    </template>
                                                </Column>
                                        ...
                                        <template v-if="showFooter && pageReady">
                                            <div class="flex justify-content-between align-items-center">
                                                <div class="flex justify-content-center flex-grow-0">
                                                    <template v-if="auth.canView('user_types/delete')">
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'user_typeslist',
		},
		apiPath: {
			type: String,
			default: 'user_types/index',
		},
		paginate: {
			type: Boolean,
			default: true,
	...
	const { load,    exportPage, debounce, clearSearch, onSort,  deleteItem, setCurrentRecord,      } = page.methods;
	
	function getActionMenuModel(data){
		return [
		{
			label: "View",
			to: `/user_types/view/${data.id}`,
			icon: "pi pi-eye",
			visible: auth.canView('user_types/view')
		},
		{
			label: "Edit",
			to: `/user_types/edit/${data.id}`,
			icon: "pi pi-pencil",
			visible: auth.canView('user_types/edit')
		},
		{
			label: "Delete",
			command: (event) => { deleteItem(data.id) },
			icon: "pi pi-trash",
			visible: auth.canView('user_types/delete')
		}
	]
	}
