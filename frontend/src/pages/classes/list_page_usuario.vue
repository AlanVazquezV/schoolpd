...
<script setup>
	...
	const props = defineProps({
		...
		pageStoreKey: {
			type: String,
			default: 'CLASSES-LIST_PAGE_USUARIO',
		},
		pageName: {
			type: String,
			default : 'classes',
		...
		routeName: {
			type: String,
			default: 'classeslist_page_usuario',
		},
		apiPath: {
			type: String,
			default: 'classes/list_page_usuario',
	...
	function getActionMenuModel(data){
		...
		},
		{
			...
			icon: "pi pi-pencil",
			visible: auth.canManage('classes/edit', data.user)
		},
		{
			label: "Delete",
			command: (event) => { deleteItem(data.id) },
			icon: "pi pi-trash",
			visible: auth.canManage('classes/delete', data.user)
		}
	]
	}
